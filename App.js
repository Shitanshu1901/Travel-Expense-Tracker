...
export default function App() {
  ...
  const handleSaveExpense = () => {
    ...
    setEditingId(null); setAmount1(''); setDescription(''); setIsSplit(false); setCustomCategory('');
    Keyboard.dismiss();
  };
  ...
  const renderHome = () => (
    <ScrollView ref={scrollRef} stickyHeaderIndices={[0]}>
      <View style={styles.header}>
        ...
        <View style={styles.homeCurrencyRow}>
            ...
            <View style={styles.currencyPickerWrapper}>
              <Picker style={{color:'#000'}} selectedValue={masterCurrency} onValueChange={(m) => { setMasterCurrency(m); saveData(trips, activeTrip, m, tripBudgets); }}>
        ...
        <View style={styles.row}>
          <View style={styles.tripPicker}><Picker style={{color:'#000'}} selectedValue={activeTrip} onValueChange={setActiveTrip}>{Object.keys(trips).map(t => <Picker.Item key={t} label={t} value={t} />)}</Picker></View>
      ...
      <View style={styles.inputCard}>
        {editingId && <Text style={{color: '#3b82f6', fontWeight: 'bold', marginBottom: 5}}>Editing Mode</Text>}
        <View style={styles.row}>
        ...
        <View style={styles.row}>
          <View style={styles.halfPicker}><Picker style={{color:'#000'}} selectedValue={category} onValueChange={setCategory}>{CATEGORIES.map(c => <Picker.Item key={c} label={c} value={c} />)}</Picker></View>
        ...
        <View style={[styles.row, {marginTop:10}]}>
          <View style={styles.halfPicker}><Picker style={{color:'#000'}} selectedValue={currency1} onValueChange={setCurrency1}>{CURRENCIES.map(c => <Picker.Item key={c.value} label={c.label} value={c.value} />)}</Picker></View>
          <View style={styles.halfPicker}><Picker style={{color:'#000'}} selectedValue={paymentMethod} onValueChange={setPaymentMethod}>{PAYMENTS.map(p => <Picker.Item key={p} label={p} value={p} />)}</Picker></View>
        </View>
        ...
        <TouchableOpacity style={styles.submitBtn} onPress={handleSaveExpense}><Text style={styles.btnText}>{editingId ? 'UPDATE EXPENSE' : '+ ADD EXPENSE'}</Text></TouchableOpacity>
      </View>
  ...
  const renderCharts = () => (
    <ScrollView style={{flex:1, padding: 20}}>
      ...
      <TouchableOpacity style={styles.submitBtn} onPress={() => {}}><Text style={styles.btnText}>REPORT EXPORT COMING TO TABS SOON</Text></TouchableOpacity>
    </ScrollView>
  );
      ...
      <Modal visible={modalVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}><View style={styles.modalContent}>
          ...
          <View style={styles.row}>
            ...
            <TouchableOpacity style={styles.modalBtn} onPress={handleTripSubmit}><Text style={{color:'#fff'}}>Save</Text></TouchableOpacity>
          </View>
        </View></View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  budgetContainer: { marginTop: 10, marginBottom: 10 },
